...
Physical-disk sample timestamps remain UTC, but rate denominators use monotonic elapsed time so system-clock adjustments cannot create a negative rate or spike. Confirmed ETW loss or queue overflow permanently marks the current session totals as lower bounds until collector state resets.
