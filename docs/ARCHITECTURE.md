...
4. `EtwPhysicalDiskEventSource` owns one shared real-time kernel session named `MonitoringXS.KernelMetrics.v1` with disk-I/O, thread, and network TCP/IP keywords. Physical-disk and network events use separate bounded queues. ETW callbacks only normalize and enqueue data; they never wait for a consumer. The bounded IRP map retains the original user initiator across multiple split completions and does not let a later PID 4 split-init overwrite that evidence.
